...
s_realdft_1d (minfft_real *x, minfft_cmpl *z, int sz, const minfft_aux *a) {
	...
	minfft_real *zi=zr+1;
	register minfft_real ur,vr,ui,vi;
	minfft_real *tr,*er,*ti,*ei;
	if (N==1) {
		/* trivial case */
		zr[0]=x[0];
		zi[0]=0;
...
s_dct3_1d (minfft_real *x, minfft_real *y, int sy, const minfft_aux *a) {
	...
	/* prepare sub-transform inputs */
	er=(minfft_real*)e,zr=(minfft_real*)z;
	ei=er+1,zi=zr+1;
	/* z[0]=x[0]; */
	zr[0]=x[0];
	zi[0]=0;
...
minfft_mkaux_realdft_1d (int N) {
	xil_printf("enering mkaux\n\r");
	minfft_aux *a;
	int n;
	minfft_real *e;
	if (N<=0 || N&(N-1)){
		xil_printf("negative or not power of two\n\r");
		/* error if N is negative or not a power of two */
		return NULL;
	}
	a=malloc(sizeof(minfft_aux));
	...
	if (N>=4) {
		...
		if (a->t==NULL){
			xil_printf("t malloc failed\n\r");
			goto err;
		}
		a->e=malloc((N/4)*sizeof(minfft_cmpl));
		if (a->e==NULL){
			xil_printf("e malloc failed \n\r");
			goto err;
		}
		e=(minfft_real*)a->e;
		for (n=0;n<N/4;++n) {
